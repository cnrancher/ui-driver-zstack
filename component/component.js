...
import _parse from 'shared/utils/parse-unit'
...
const formatSi      = _parse.formatSi;
/*!!!!!!!!!!!GLOBAL CONST END!!!!!!!!!!!*/
...
export default Ember.Component.extend(NodeDriver, {
  ...
  actions: {
    zstackLogin: function (cb) {
      ...
      let proxyEndpoint = get(this,'app.proxyEndpoint') + '/' + endpoint.replace('//', '/');
      ...
      Promise.all([this.sendPut('zstack/v1/accounts/login', {
        logInByAccount: {
    ...
    selectNetwork: function (cb) {
      ...
      return Promise.all([instancePromise, imagePromise, systemDiskPromise, physicalHostPromise, primaryStoragePromise]).then(() => {
        set(this, 'step', 3);
        cb();
      });
    }
